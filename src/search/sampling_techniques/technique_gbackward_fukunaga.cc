...
const string &TechniqueGBackwardFukunaga::get_name() const {
    return name;
}
...
static shared_ptr<TechniqueGBackwardFukunaga> _parse_technique_gbackward_fukunaga(
    ...
    if (!parser.dry_run()) {
        technique = make_shared<TechniqueGBackwardFukunaga>(opts);
    }
    return technique;
}

static Plugin<SamplingTechnique> _plugin_technique_gbackward_fukunaga(
        TechniqueGBackwardFukunaga::name, _parse_technique_gbackward_fukunaga);
}
