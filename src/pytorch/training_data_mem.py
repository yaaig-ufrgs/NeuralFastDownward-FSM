...
"""
A more memory-efficient (but slower) InstanceDataset.
"""
...
class InstanceDataset(Dataset):
    def __init__(
        ...
        if output_layer not in ["regression", "prefix", "one-hot"]:
            raise RuntimeError(f"Invalid output layer: {output_layer}")
        self.output_layer = output_layer
        self.has_weights = False if len(weights) == 0 else True
        self.domain_max_value = domain_max_value

        self.weights = weights
        self.states = states
        self.hvalues = heuristics

    def __getitem__(self, idx: int) -> (torch.Tensor, torch.Tensor, torch.Tensor):
        s = torch.tensor(np.array(np.fromiter(self.states[idx], dtype=np.int8)), dtype=torch.float32)
        h_val = [self.hvalues[idx]]

        if self.output_layer == "regression":
            h = torch.tensor(
                np.array(h_val), dtype=torch.float32
            )
        elif self.output_layer == "prefix":
            h = torch.tensor(
                np.array(to_prefix(h_val, self.domain_max_value)),
                dtype=torch.float32,
            )
        elif output_layer == "one-hot":
            h = torch.tensor(
                np.array(to_onehot(h_val)),
                dtype=torch.float32,
            )
        if self.has_weights:
            w_val = [self.weights[idx]]
            w = torch.tensor(
                np.array(w_val), dtype=torch.float32
            )
            return s, h, w
    ...
    def y_shape(self):
        if self.output_layer == "prefix":
            return (len(self.hvalues), to_prefix(self.hvalues[0]))
        elif self.output_layer == "one-hot":
            return (len(self.hvalues), to_onehot(self.hvalues[0]))
        return (len(self.hvalues), 1)
