...
from src.pytorch.utils.helpers import pair_to_lists
...
class KFoldTrainingData:
    def __init__(
        ...
        self.state_value_pairs, self.domain_max_value = \
            load_training_state_value_pairs(samples_file)
    ...
    def generate_kfold_training_data(self):
        ...
        for i in range(self.num_folds):
            ...
            if self.model == "rsl":
                states, heuristics = pair_to_lists(self.state_value_pairs)
                x_train, x_valid, y_train, y_valid = train_test_split(np.array(states), np.array(heuristics).reshape((-1,1)), test_size=0.2, shuffle=True, random_state=self.seed)
                x_train, x_valid, y_train, y_valid = torch.tensor(x_train, dtype=torch.float), torch.tensor(x_valid, dtype=torch.float), torch.tensor(y_train, dtype=torch.float), torch.tensor(y_valid, dtype=torch.float)

                training_set = torch.utils.data.TensorDataset(x_train, y_train)
                test_set = torch.utils.data.TensorDataset(x_valid, y_valid)

                train_dataloader = DataLoader(
                    dataset=training_set,
                    batch_size=self.batch_size,
                    shuffle=self.shuffle, num_workers=1,)
...
def seed_worker_rsl(seed=80970):
    print("seed: ", seed)
    np.random.seed(seed)
    random.seed(seed)
