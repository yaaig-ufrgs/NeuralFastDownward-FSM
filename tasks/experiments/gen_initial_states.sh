...
../../fast-downward.py $SOURCE_PDDL --translate-options --keep-unimportant-variables --search-options --search "generator(techniques=[iforward_none($N,distribution=uniform_int_dist($RANDOM_STEPS,$((RANDOM_STEPS+1)),random_seed=$RANDOM_SEED_LENGTH),random_seed=$RANDOM_SEED_WALK)])" > /dev/null
